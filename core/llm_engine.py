...
class LLMEngine:
    ...
    def __init__(self, model_name: str, tier: str, temperature: float = 0.5, tool_temperature: float = 0.1, history_limit: int = 10):
        ...
        self.history = []
        logging.info(f"Zainicjalizowano LLMEngine: Model={model_name}, Tier={self.tier}, Temp={temperature}, ToolTemp={tool_temperature}, HistoryLimit={history_limit}")
    ...
    def generate_response(self, prompt: str, tools_registry, status_callback: Any = None, stream_callback: Any = None) -> str:
        ...
        """
        # System Prompt budowany dynamicznie na podstawie aktywnej warstwy i plików
        ...
        while True:
            current_temp = self.tool_temperature if is_tool_pass else self.temperature
            ...
            try:
                ...
                for line in response.iter_lines():
                    ...
                    if "content" in msg_chunk and msg_chunk["content"]:
                        ...
                        full_content += piece
                        if stream_callback:
                            stream_callback(piece, is_tool_pass)
                ...
                if is_tool_pass and not tool_calls_accumulator and response_text:
                ...
                if is_tool_pass and not tool_calls_accumulator:
                    if full_content.strip():
                        now_assistant = datetime.datetime.now().strftime("%H:%M:%S")
                        self.history.append({
                            "role": "assistant",
                            "content": full_content.strip(),
                            "timestamp": now_assistant,
                            "is_internal": True
                        })
                    is_tool_pass = False
                    continue
                ...
                if tool_calls_accumulator:
                    ...
                    for tool_call in tool_calls_accumulator:
                        ...
                        # Przekazanie bezpiecznego dict'a do funkcji execute_tool
                        arguments = args_dict
