...
- **Modele Instruct (qwen2.5-instruct)**: Projekt używa wariantów `-instruct` modeli Qwen 2.5 (`qwen2.5:1.5b-instruct` dla Lokaja na Raspberry Pi, `qwen2.5:14b-instruct` dla Regisa). Modele Instruct oferują znacznie lepsze instruction following, tool calling i reasoning niż warianty bazowe. Półka modeli 1.5B została oficjalnie adoptowana dla pierwszej warstwy w celu uzyskania bardzo szybkiej generacji dla systemów głosowych (TTS/STT). Temperatura: Lokaj=0.1, Regis=0.4.
...
- **NLU Structured Outputs dla modelu Butler (1.5B)**: Zamiast klasycznej, powolnej pętli ReAct obciążonej ryzykiem halucynacji narzędzi u tak małego modelu, dla tieru Butler wprowadzono dedykowaną, "szybką ścieżkę". Korzystając ze wsparcia API Ollamy dla wymuszania formatowania (JSON Schema `format`), model działa jak klasyczny parser Natural Language Understanding. Zwraca w deterministycznie ułamku sekundy gotowy obiekt JSON określający intencję np. `light_on`, omijając "angielski drift" promptów i znacznie odciążając hardware na Raspberry Pi.
- **Wysokowydajny HomeAssistantClient**: Całkowicie zoptymalizowano obsługę API Home Assistanta pod kątem działania z Raspberry Pi. Klient zachowuje długotrwałe połączenia w ramach puli `requests.Session()` usuwając narzuty TLS. Obsługuje również potężny system **Wirtualnych Grup** — umożliwia to zdefiniowanie logicznej grupy wielu urządzeń (np. 7 żarówek jako jeden obiekt `light.moj_pokoj`) i wysterowanie ich naraz jednym żądaniem sieciowym, zamiast kosztownych synchronicznych pętli. Zrezygnowano też z odpytywania HA o stan całości urządzeń w domu przed wykonaniem każdej akcji, co ucięło wielosekundowe czasy oczekiwania.
- **Separation of Concerns (Dynamic Prompts)**: Instrukcje psychologiczne agentów są doczytywane dynamicznie z plików `data/prompts/` (`base_system.md`, `tier_butler.md`, `tier_regis.md`). Pliki są w formacie wypunktowanych list, co optymalnie współpracuje z modelami Instruct. Prompt systemowy jest ściśle dopasowany ("tier-aware") do posiadanych przez dany profil narzędzi. Dla tieru Butler (NLU) prompt zredukowano do ekstremum opartego o technikę Few-Shot JSON.
...
- **Scentralizowany System STT (Thin Client)**: Architekturę przetwarzania głosu oparto o "Cienkiego Klienta". Moduł Satelity nagrywa jedynie dźwięk i wysyła surowy plik `.wav` pod endpoint `/v1/chat/audio_stream` do serwera (FastAPI). Serwer główny (Malinka) przechowuje załadowany na stałe model transkrypcyjny (`faster-whisper`) w pamięci RAM obok modelu językowego (LLM), co całkowicie zdejmuje narzuty sprzętowe i logiczne z urządzeń peryferyjnych-satelitów, pozwalając na użycie tanich mikrokontrolerów.
