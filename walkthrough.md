...
- **Pamięć Długoterminowa (Notatnik) i Brudnopis (Staging)**: Narzędzia do przeszukiwania (`search_memory`), zapisu (`save_memory`) i usuwania. Model może sam konsolidować brudnopis dzięki narzędziom `get_pending_notes` (zwracającym bezpośrednio pełne dane - brak antywzorca stanowego "desk_state") i operacjom atomowym `archive_note` (równoczesny zapis pamięci z destrukcją bufora).
