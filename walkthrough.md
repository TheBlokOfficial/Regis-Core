...
- **Wewnętrzny Monolog `<thought>` (Real-Time Streaming)**: Model ma obowiązek pisać tok rozumowania w tagach `<thought>...</thought>` przed każdą akcją lub odpowiedzią. CLI parsuje te tagi token po tokenie (maszyna stanów ze sliding window buforem) i wyświetla zawartość w czasie rzeczywistym jako szary tekst "Myśli agenta:". JSON-y tool-callów i śmieciowe znaczniki Qwen są filtrowane i nigdy nie trafiają na ekran.
...
- **Separation of Concerns (Dynamic Prompts)**: Instrukcje psychologiczne agentów są doczytywane dynamicznie z plików `data/prompts/` (`base_system.md`, `tier_butler.md`, `tier_regis.md`). Pliki są w formacie wypunktowanych list, co optymalnie współpracuje z modelami Instruct.
